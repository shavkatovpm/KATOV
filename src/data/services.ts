    ...
    content: {
      uz: {
        ...
        deliverables: [
          ...
          'AI uchun maxsus "yo\'l ko\'rsatkichi" yaratamiz — ChatGPT yoki Claude saytingizga kirganda kerakli ma\'lumotni darrov topadi (texnik tomon: llms.txt)',
          'Savol-javob bo\'limini AI yoqtirdigan qilib qayta yozamiz — foydalanuvchi ChatGPT\'da savol bersa, javob sizning saytingizdan olinadi',
          'Brendingizni AI uchun "rasmiy entity" qilamiz — Wikidata\'da yozuv yaratamiz va Wikipedia uchun submission tayyorlaymiz (qabul qilinishi Wikipedia editorlari qaroriga bog\'liq)',
          'AI\'ga "siz mutaxassizsiz" degan signal beramiz — kompaniya tarixi, tajriba va kreditlaringiz saytda to\'g\'ri ko\'rinadi (E-E-A-T mezonlari)',
          'Hamma AI\'larning saytingizga kirishi ta\'minlanadi — ChatGPT, Claude, Perplexity, Google AI bot\'lari hech biri bloklanmaydi (robots.txt sozlash)',
          'AI iqtibos qilishi oson kontent yozamiz — aniq raqamlar, foydali ma\'lumot, manba ko\'rsatilgan format. AI bunday matnni yoqtiradi',
          'Saytingiz o\'z mavzusi bo\'yicha "ekspert manba" sifatida tanitiladi — bir mavzuda chuqur, bog\'langan kontent AI\'ga "bu yerda haqiqiy bilim bor" degan signal beradi',
          'Reddit va Quora\'da brendingiz haqida nima yozilayotganini kuzatamiz — AI bu saytlardan ko\'p iqtibos oladi, e\'tibor zarur',
          'Har oy ChatGPT, Perplexity va Gemini\'da test qilamiz — sizning brendingiz qaysi savollarga javobda chiqdi, qaysi raqobatchi bilan birga ko\'rindi',
          'Uzbek, Rus va Ingliz tillarida — uchchasida ham AI\'da chiqasiz, har til uchun alohida strategiya bilan',
          'Oylik hisobotni sizga tushunarli tilda taqdim etamiz — qaysi AI sizni eslatdi, qaysi savolda, raqobatchilarga nisbatan qayerdasiz',
        ],
        processTitle: 'Jarayon',
      ...
      ru: {
        ...
        deliverables: [
          'Полная проверка готовности сайта к AI — находим, почему ChatGPT/Perplexity сейчас не упоминают ваш бренд',
          'Делаем так, чтобы AI правильно понимал ваши данные — цена, адрес, услуги, контакты размечены явно для AI (технически: schema.org-разметка)',
          'Создаём для AI отдельный "путеводитель" — когда ChatGPT или Claude заходит на сайт, он быстро находит нужную информацию (технически: файл llms.txt)',
          'Переписываем раздел вопрос-ответ в формате, который любит AI — когда пользователь спрашивает в ChatGPT, ответ берётся именно с вашего сайта',
          'Делаем ваш бренд "официальной entity" для AI — создаём запись в Wikidata и готовим submission для Wikipedia (приём зависит от решения редакторов Wikipedia)',
          'Передаём AI сигнал "вы — эксперты" — история компании, опыт и подтверждения отображаются на сайте корректно (критерии E-E-A-T)',
          'Открываем сайт для всех AI — боты ChatGPT, Claude, Perplexity и Google AI не блокируются (настройка robots.txt)',
          'Пишем контент, который AI удобно цитировать — конкретные цифры, полезная информация, указанные источники. AI выбирает именно такой формат',
          'Сайт начинают воспринимать как "экспертный источник" в своей теме — глубокий, связанный контент по одной теме говорит AI: "тут реальное знание"',
          'Мониторим, что пишут о вашем бренде в Reddit и Quora — AI часто цитирует эти источники, важно отслеживать',
          'Каждый месяц тестируем вас в ChatGPT, Perplexity и Gemini — показываем, в каких вопросах появлялся ваш бренд и рядом с какими конкурентами',
          'На узбекском, русском и английском — везде вы в AI, со своей стратегией для каждого языка',
          'Ежемесячный отчёт — понятным языком: какой AI вас упомянул, по какому вопросу, как вы выглядите рядом с конкурентами',
        ],
        processTitle: 'Процесс',
      ...
      en: {
        ...
        deliverablesTitle: 'What\'s included',
        deliverables: [
          'A full check of how AI-ready your site is — we find out why ChatGPT/Perplexity aren\'t mentioning your brand yet',
          'We make sure AI understands your data correctly — price, address, services, contacts each explicitly labeled for AI (technical layer: schema.org markup)',
          'A dedicated "guide" for AI — when ChatGPT or Claude visits your site, it quickly finds the right info (technical layer: llms.txt file)',
          'We rewrite your FAQ section the way AI likes it — when a user asks ChatGPT, your site\'s answer is the one that gets picked',
          'We make your brand an "official entity" for AI — Wikidata entry creation, Wikipedia submission preparation (acceptance depends on Wikipedia editors)',
          'We send AI the "you\'re the experts" signal — company history, experience, and credentials are displayed correctly on your site (E-E-A-T criteria)',
          'We open your site to every AI — ChatGPT, Claude, Perplexity, and Google AI bots are all allowed (robots.txt configuration)',
          'We write content that\'s easy for AI to cite — specific numbers, useful info, named sources. AI prefers exactly this format',
          'Your site is positioned as an "expert source" in its topic — deep, connected content on a single topic tells AI: "real knowledge is here"',
          'We monitor what\'s being said about your brand on Reddit and Quora — AI cites these sources often, worth tracking',
          'Every month we test you in ChatGPT, Perplexity, and Gemini — showing which questions surfaced your brand and next to which competitors',
          'In Uzbek, Russian, and English — you appear in AI across all three, with a dedicated strategy per language',
          'A monthly report in plain language — which AI mentioned you, on what question, and how you compare to competitors',
        ],
        processTitle: 'Process',
